...
async function setup(){
  app.innerHTML='<div class="intro"><section class="intro-copy"><p class="eyebrow">YOUR FRIENDS. YOUR CATEGORIES.</p><h1>Make it<br>a <span class="gold">quiz night.</span></h1><p class="muted">Five categories. Twenty-five questions.<br>One very competitive group of friends.</p><div class="mini-board" aria-hidden="true"><span>200</span><span>400</span><span>600</span><span>800</span><span>1,000</span></div><div class="card"><h3>How the night works</h3><p class="help">Teams take turns picking a question and answer out loud. The host judges. Miss it? Another team gets one chance to steal.</p><p class="help">Two hidden Double or Nothing questions let the selecting team choose normal or double stakes. Double stakes can win or lose twice the points. No steals.</p></div><form id="join" class="join"><label for="code">Already playing? Enter the game code</label><div class="actions"><input id="code" name="code" placeholder="16-character code" required maxlength="16" pattern="[a-fA-F0-9]{16}" autocomplete="off"><button>Join game</button></div></form></section><form id="setup" class="card setup"><p class="eyebrow">SET THE STAGE</p><h2>Create a game</h2><label for="title">Game title</label><input id="title" name="title" value="Rahul’s Bachelor Party" maxlength="100" required><label for="teams">Teams · one per line</label><textarea id="teams" name="teams" required>Team One\nTeam Two\nTeam Three</textarea><p class="help">Choose 2–8 teams. Teams select in the order listed.</p><label for="password">Host password</label><input id="password" name="password" type="password" minlength="8" maxlength="128" required autocomplete="new-password"><p class="help">Keep this password for host access on any device.</p><label for="pack">Question pack · JSON file</label><input id="pack" type="file" accept=".json,application/json"><p id="pack-status" class="help">Using the starter pack: Berkeley, San Francisco, Random, Cupertino, and Wildcard.</p><button type="button" id="template">Download editable starter pack</button><details><summary>Question file format</summary><p class="help">Five categories, each with a name and five clues. Each clue needs question, answer, points, and special. Mark exactly two clues special.</p><pre>[{\n  "name": "Category name",\n  "clues": [{\n    "question": "Your question?",\n    "answer": "Accepted answer",\n    "points": 200,\n    "special": false\n  }]\n}]</pre><p class="help">The example shows one entry. Download the complete template above.</p></details><label><input id="party" type="checkbox" checked>Enable team-shot questions</label><label for="shots">Number of hidden shot questions</label><input id="shots" type="number" min="1" max="23" value="4" required><p class="help">Randomly chosen each game, separate from Double or Nothing. The selecting team sees a “Team shot!” prompt before the question. Participation is optional; any beverage works.</p><button class="primary" id="create">Create game</button><p class="help">Keep the game link to return later. Progress saves automatically.</p></form></div>';
...
function draw(){
  ...
  let html='<section class="topline"><div><p class="eyebrow">QUIZ NIGHT <span class="pill">'+(s.host?'HOST':'PLAYER VIEW')+'</span></p><h1 class="game-name">'+esc(s.title)+'</h1><span class="muted">'+s.used.length+' / 25 questions complete</span></div><div class="actions"><button id="copy">Copy game link</button>'+(s.host?'<button id="logout">Player view</button>':'<button class="primary" id="unlock">Host sign in</button>')+'</div></section>';
  if(finished){
    const top=Math.max(...s.teams.map(t=>t.score)), winners=s.teams.filter(t=>t.score===top).map(t=>t.name);
    html+='<section class="win"><p class="eyebrow">THAT’S A WRAP</p><h2>'+esc(winners.join(' & '))+(winners.length>1?' tie!':' wins!')+'</h2><p>'+top.toLocaleString()+' points · Thanks for playing.</p><a href="/" class="button primary">Create another game</a></section>';
  }else{
    ...
    html+='<div class="status"><strong>'+esc(status)+'</strong><span class="muted">'+(s.host?'You control the board and scoring.':'Answer out loud. Your host controls the game.')+'</span></div>';
  }
  if(a){
    html+='<section class="clue"><p class="eyebrow">'+esc(a.category)+' · '+a.points.toLocaleString()+' POINTS'+(a.multiplier===2?' · DOUBLE STAKES':'')+'</p>';
    if(a.phase==='shot'){
      html+='<h2>Team shot!</h2><p class="answer">'+esc(who)+', this one’s for you.</p><p class="muted">Take a shot together, choose any beverage, or sit this one out.</p>'+(s.host?button('Continue to question','continue','class="primary"'):'<p>Waiting for the host to open the question.</p>');
      ...
      if(a.phase==='resolved') html+='<p class="answer">'+esc(a.answer)+'</p><p>'+esc(a.result)+'</p>'+(s.host?button(s.used.length===24?'Finish game':'Back to board · next team','next','class="primary"'):'<p class="muted">Waiting for the host to continue.</p>');
        ...
        if(s.host) html+='<div class="host-answer"><small>HOST ONLY · ACCEPTED ANSWER</small><p>'+esc(a.answer)+'</p></div>';
  ...
  if(s.host) html+='<div class="host-tools"><div class="actions">'+button('Undo last action','undo',s.canUndo?'':'disabled')+'<button id="export">Download this question pack</button></div><p class="help">Open the player link on a separate device for the audience. This screen contains host-only answers.</p></div>';
  app.innerHTML=html;
...
async function refresh(){
  ...
  try{
    const result=await api('/api/games/'+id);
    connection.textContent='Live · progress saved';
...
document.querySelector('#login-form').onsubmit=async e=>{
  ...
  try{await api('/api/games/'+id+'/login',{password:new FormData(e.target).get('password')});e.target.reset();document.querySelector('#login').close();last='';await refresh();}
